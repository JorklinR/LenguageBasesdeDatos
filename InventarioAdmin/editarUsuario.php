...
<body>
    ...
    <form action="acciones/actualizarUsuario.php" method="post">
...
if ($resultado->num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        <input type="text" name="nombre" id="nombre" placeholder="Digite el nombre completo" value="' . $row['nombre_completo'] . '">
    </div>
    
    <div>
        <label id="titulo2" for="correo">Correo:</label>
        <input type="email" name="correo" id="correo" placeholder="Digite el correo" value="' . $row["correo"] . '">
    </div>
    
    <div>
        <label id="titulo2" for="usuario">Usuario:</label>
        <input type="text" name="usuario" id="usuario" placeholder="Digite el usuario" value="' . $row['usuario'] . '">
    </div>

    <div>
        <label id="titulo2" for="rol">Rol:</label>
        <input type="number" name="rol" id="rol" placeholder="Digite el rol que posee" value="' . $row['rol_id'] . '">
    </div>
